...
function boykov_kolmogorov_cut(A::AbstractArray, B::AbstractArray, dir::Symbol)
  ...
  for k=1:mz, j=1:my, i=1:mx-1
    c = cart2lin((mx,my,mz), i, j, k)
    d = cart2lin((mx,my,mz), i+1, j, k)
    add_edge!(G, c, d)
    add_edge!(G, d, c)
  ...
  for k=1:mz-1, j=1:my, i=1:mx
    c = cart2lin((mx,my,mz), i, j, k)
    o = cart2lin((mx,my,mz), i, j, k+1)
    add_edge!(G, c, o)
    add_edge!(G, o, c)
    C[c,o] = C[o,c] = (E[c] + E[o]) / (∇zₐ[c] + ∇zₐ[o] + ∇zᵦ[c] + ∇zᵦ[o])
  end
  for k=1:mz, j=1:my
    u = cart2lin((mx,my,mz), 1, j, k)
    v = cart2lin((mx,my,mz), mx, j, k)
    add_edge!(G, s, u)
    add_edge!(G, v, t)
    C[s,u] = C[v,t] = Inf
  end
